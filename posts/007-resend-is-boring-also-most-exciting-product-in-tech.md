...
It's a narrow dataset, but the signal is worth considering: AI assistants may not just be mentioning tools but they could be actively favoring the ones with cleaner developer experience.
...
These tools don't just help write code but they make infrastructure decisions. When a developer asks Cursor or Claude to "add email functionality," the AI picks the service, writes the integration, and often runs the install command.
...
**4. Parseable documentation.** Not just well-written docs but docs structured in ways that AI can extract and act on. Clear headings, code blocks, consistent formatting.
